...
export const useGetValidationInfo = ({ openSnackbar, closeModal }: IUseGetValidationInfo) => {
	const initialValue = {
		name: '',
		message: '',
		email: '',
	...
	const {
		...
		validationSchema: Yup.object().shape({
			email: Yup.string()
				.email('Invalid email')
				.matches(/^[A-Z0-9_%+-]+(\.[A-Z0-9_%+-]+)*@[A-Z0-9.-]+\.[A-Z]{2,4}$/i, 'Invalid email')
				.required('Required'),
			name: Yup.string()
				.min(2, 'Minimum 2 characters')
				.max(20, 'Maximum 20 characters')
				.matches(/^((?:([a-zA-Z]+[\s'.-]*)+))$/, 'Latin, space, hyphen, apostrophe, period allowed')
				.required('Required'),
			message: Yup.string()
